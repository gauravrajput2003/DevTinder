...
userRouter.get("/user/connections",userAuth,async(req,res)=>{
    try{
        console.log("=== CONNECTIONS ROUTE CALLED ===");
        ...
        console.log("Found connection requests:", connectionRequest.length);
        
        const data=connectionRequest.map((row)=>{
            console.log("Row:", {
                fromUserId: row.fromUserId?._id,
...
userRouter.get("/user/:userId",userAuth,async(req,res)=>{
    try{
        ...
        const user = await User.findById(userId).select("firstName lastName photoUrl gender age about skills");
